...
ADD_PHOTO <- function(input, output, session, site, photo_date, mod_loc, par = NULL, photos, globalSession, mode = reactive(2)) {
...
uploadPhoto <- function(file) {
  ...
  if(file.exists(file)){
    ...
    GS_APPEND_PHOTO(sheet = config[14], data = photo_rec, photos = photos_db)
    return("Photo uploaded to Dropbox and record appended successfully")
  } else {
